...
def male_anxiety_scatterplot():
    ...
    # Prepare data for the scatter plot
    x = male_data['Anxiety Level (1-10)']
    y = male_data['Caffeine Intake (mg/day)']
...
@app.route('/female-heart-scatterplot')
def female_heart_scatterplot():
    # Filter data for Gender == Male
    female_data = df[df['Gender'] == 'Female']

    # Prepare data for the scatter plot
    x = female_data['Anxiety Level (1-10)']
    y = female_data['Heart Rate (bpm)']
    ...
    plt.title('Heart Rate (bpm) vs Anxiety Level for Females')
    plt.grid(True)
...
@app.route('/male-heart-scatterplot')
def male_heart_scatterplot():
    # Filter data for Gender == Male
    male_data = df[df['Gender'] == 'Male']

    # Prepare data for the scatter plot
    x = male_data['Anxiety Level (1-10)']
    y = male_data['Heart Rate (bpm)']
    ...
    plt.xlabel('Anxiety Level (1-10)')
    plt.ylabel('Heart Rate (bpm)')
    plt.title('Heart Rate (bpm) vs Anxiety Level for Males')
    plt.grid(True)
